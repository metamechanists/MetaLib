...
public class GlowingEntities implements Listener {
    ...
    /**
     * Check if an entity is glowing for a player
     *
     * @param entity entity to check if glowing
     * @param receiver player to check if the entity is glowing for
     * @return {@link Boolean} if the entity is glowing for the player
     */
    public boolean isGlowing(Entity entity, Player receiver) {
        ensureEnabled();
        return glowing.get(receiver) != null && glowing.get(receiver).glowingDatas.containsKey(entity.getEntityId());
    }
